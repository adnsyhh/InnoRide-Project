...
class _DetailVehicleScreenState extends State<DetailVehicleScreen> {
  ...
  void fetchVehicleDetail() async {
    try {
      ...
      final allReviews = await ApiService.getReviews(widget.vehicleId);

      // ⬇️ Ini cara benar bikin filteredReviews
      final List<dynamic> filteredReviews =
          allReviews.where((review) {
            return review['vehicle_id'] == widget.vehicleId;
          }).toList();
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          '${(vehicleData?['rating'] ?? 5.0).toStringAsFixed(1)} (${vehicleData?['review_count'] ?? 2} reviews)',
